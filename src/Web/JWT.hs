...
Known limitations:
   ...
   * Only HMAC SHA-256 ("HS256") and RSASSA-PKCS1-v1_5 with the SHA-256 hash ("RS256") are currently supported signature algorithms
...
data Secret = RawSecret T.Text
            | RSASecret RSA.PublicKey

instance Eq Secret where
    (RawSecret s1) == (RawSecret s2) = s1 `T.constTimeCompare` s2
    _ == _ = error "Can only compare raw secrets"
...
verify secret' (Unverified header'@(JWTHeader {alg=(Just HS256)}) claims' unverifiedSignature originalClaim) = do
...
rsaPublicKeySecret :: RSA.PublicKey -> Secret
rsaPublicKeySecret = RSASecret
...
calculateDigest HS256 (RawSecret key) msg = base64Encode' $ HMAC.hmac SHA.hash 64 (bs key) (bs msg)
    where bs = TE.encodeUtf8
calculateDigest _ _ _ = error "Can only compute digest for HS256 and raw secrets"
